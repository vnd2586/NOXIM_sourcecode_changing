...
//namdv2 removed from here
    signal(SIGQUIT, signalHandler);  
//namdv2 remeved end here

    // TEMP
    drained_volume = 0;

    // Handle command-line arguments
//    cout << endl << "\t\tNoxim - the NoC Simulator" << endl;//namdv3
//    cout << "\t\t(C) University of Catania" << endl << endl; //nadv3
    ...
    reset.write(1);
//    cout << "Reset..." << endl;//namdv4
    srand(GlobalParams::rnd_generator_seed);	// time(NULL));


    sc_start(GlobalParams::reset_time, SC_NS);
    reset.write(0);
//    cout << " done! Now running for " << GlobalParams:://namdv5
//	simulation_time << " cycles..." << endl;//namdv6
    sc_start(GlobalParams::simulation_time, SC_NS);

    // Close the simulation
    if (GlobalParams::trace_mode)
	sc_close_vcd_trace_file(tf);
//    cout << "Noxim simulation completed." << endl;//namdv7
//    cout << " ( " << sc_time_stamp().to_double() / GlobalParams::clock_period_ps << " cycles executed)" << endl;//namdv8
