...
*/
//namdv_add from here
	out<<"#TRP\tTRF \tGAD (cycles) \tGAT (flits/cycle) \tT (flits/cycle/IP) \tMD(cycles) \tTotal Power (J) \tDynamic Energy (J) \tStatic Energy (J)"<<endl;
    out <<getReceivedPackets() <<"\t"<<getReceivedFlits()<<"\t"<<getAverageDelay()<<"\t\t"<<getAverageThroughput()<<"\t\t"<<getThroughput()<<"\t\t"<<getMaxDelay()<<"\t\t"<<getTotalPower()<<"\t\t"<<getDynamicPower()<<"\t\t"<<getStaticPower()<< endl;
//namdv_add end here
